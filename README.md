...
* 만약 메세지를 보내고 곧바로 다시 이 메서드를 실행한다면? 오름차순으로 이벤트를 가져오기에 응답을 받지 못한 이벤트(PENDING)에 대해서 다시 메세지를 보낼 확률이 생깁니다.
* 해결3: CompletableFuture.allOf().join()을 사용합니다. 해당 배치의 모든 비동기 전송 작업이 완료(DB 상태 변경 포함)될 때까지 스케줄러 스레드를 대기시킵니다.
* 이를 통해 스케줄러 간의 실행 주기가 겹치는 것을 방지하고, 확실하게 처리된 후 다음 배치가 실행되도록 보장합니다. 이렇게 하면 같은 이벤트에 대해 중복 메시지 발행을 방지할 수 있습니다.
