...
7. [카프카 브로커로부터 받는 응답 수신 방법을 동기에서 비동기로 전환](#카프카-브로커로부터-응답-수신을-동기에서-비동기로-전환)
8. [API 명세서](#api-명세서)
...
* 문제1: 로컬 개발 환경과 배포 환경이 달라서 발생하는 문제(버전 불일치, 환경 변수 등)가 발생할 수 있습니다.
* 해결1: 이를 막기 위해 환경 격리을 격리하고 자원을 할당하는 컨테이너 기술 Docker를 도입합니다.

* 문제2: 여러 서비스를 다른 컨테이너에 띄우다보니 명령형 방식으로 도커 컨테이너를 하나하나 띄우는 것은 비효율적이고 실수할 확률이 있습니다.
* 해결2: 이를 막기 위해 Docker-Compose를 도입합니다. 또한 도커 네트워크를 직접 만들 필요가 없어 편리합니다.

* 문제3: Docker-Compose는 한 호스트 머신에서만 동작이 가능하기에 나중에 배포할 때 분산환경에서 동작할 수 없습니다.
* 해결3: 분산환경에서도 동작할 수 있게 하기 위해서 k8s를 도입합니다.
...
#### 테스트 시나리오
1.  **동시성 부하:** 5,000명의 가상 유저가 `/api/orders`에 주문 요청을 전송.
2.  **무작위 데이터:**
...
* 주문 성공(`SUCCESS`) 상태인 건들의 상품별 판매 수량 총합을 조회했습니다. 모든 상품이 초기 설정된 재고량인 300개에 딱 맞춰 판매되었으며, 동시성 이슈로 인한 초과 판매(Overselling)가 발생하지 않았습니다.
...
* 시스템이 셧다운되지 않고 2만 건의 요청을 받아냈습니다.
...
### 5. 톰캣 스레드, DB 커넥션 풀 튜닝 후 K6 테스트(지연 시간 11.3초 -> 4.24초)
...
테스트 시나리오: 재고가 단 1개(상품 id : 7) 남은 특정 상품에 대해 100명(VUs)이 동시에 주문을 시도.
...
## 카프카 브로커로부터 응답 수신을 동기에서 비동기로 전환

#### (과거) 문제1: DB에 주문 저장 성공, 하지만 카프카 브로커에 주문 메세지를 보냈는데 유실될 수 있습니다.
#### (문제) 해결1: 브로커에 메세지를 보내고 메세지가 도착했다는 ack를 받을 때까지 기다리는 동기 방식을 사용합니다. 이러면 메세지 전달의 신뢰성이 보장됩니다.
  
#### 문제2: 동기 방식을 사용하면 Idle 상태의 톰캣 스레드가 발생합니다.
#### 추가 정보: Outbox 패턴이 적용되어 있습니다. 주기적으로 PENDING 상태의 이벤트를 모아서 브로커에 메세지를 보내는 스케줄러가 있습니다.
#### 해결2: Outbox패턴 덕분에 그냥 비동기 방식으로 전환해도 됩니다. 만약 브로커로부터 메세지를 받는다면, 이때 이벤트 상태를 SENT로 바꿔서 메세지 전달 성공한 것으로 저장합니다.
* if 메세지가 유실되어 오랜 시간동안 메세지가 오지 않으면 => PENDING 상태일 것이기에 이것 또한 스케줄러가 처리해줄 것입니다.''

#### 문제3: findAllByStatusOrderByIdAsc(EventStatus.PENDING, pageable); 메서드는 오름차순으로 PENDING인 이벤트를 가져옵니다.
* 만약 메세지를 보내고 곧바로 다시 이 메서드를 실행한다면? 오름차순으로 이벤트를 가져오기에 응답을 받지 못한 이벤트(PENDING)를 다시 DB에서 가져와 중복된 메세지를 보낼 확률이 생깁니다.
#### 해결3: CompletableFuture.allOf().join()을 사용합니다. 해당 배치의 모든 비동기 전송 작업이 완료(카프카 메세지 응답으로 인한 DB 상태 변경 포함)될 때까지 스케줄러 스레드를 대기시킵니다.
* 이렇게 하면 같은 이벤트에 대해 중복 메시지 발행을 방지할 수 있습니다.

#### 문제4: 스케줄러가 대기(Block)하는 것은 성능 저하가 일어납니다.
#### 해결4: join()으로 스케줄러 스레드가 대기하는 것을 막아야 합니다. join()을 삭제해야 합니다.
* EventStatus에 SENDING이라는 새로운 상태를 추가합니다.
* 카프카 브로커로 메세지를 보내기 전 OutboxEvent의 EventStatus 상태를 SENDING으로 바꿉니다.
* 이렇게 하면 CompletableFuture.allOf().join()을 써서 스케줄러 스레드가 카프카 브로커로부터 스케줄러 스레드가 보낸 메세지에 대한 응답을 모두 받을 때까지 대기시키지 않아도 됩니다.
* 카프카 브로커에게 메세지 전송을 시도했다면 PENDING이 아니라 SENDING 상태이기 때문에 응답을 받지 못한 이벤트(PENDING)에 대해서 중복 메세지를 보내는 것(findAllByStatusOrderByIdAsc(EventStatus.PENDING, pageable)으로 인한)을 차단할 수 있습니다.
